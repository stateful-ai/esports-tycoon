...
choice. `/week10/prep` consumes the fallout response and writes
`week10_prep.json`, presenting an analyst-desk recommendation, spending one
scarce prep block, and carrying its prep effect into a `week10_scrim.json`
handoff. `/week10/scrim` consumes that prep block and writes
...
before Week 11 result resolution. `/week11/match/result` consumes the locked
match plan and writes `week11_match_result.json`, resolving the first Week 11
match result while stopping before post-match review.
The Week 11 prep, scrim, and match-plan screens use dedicated environment art
so the new week reads visually distinct from the Week 10 match-room flow and
each checkpoint feels like a different staff room rather than another prep
desk.
...
- **`week9_match_result.json`** — written by `/week9/match/result`, consuming
...
- **`week10_fallout.json`** — written by `/week10/fallout`, consuming the Week 9
  ...
  Week 10 prep.
- **`week10_prep.json`** — written by `/week10/prep`, consuming Week 10 fallout
  into an in-universe analyst packet, one prep-block allocation, prep-effect
  deltas, and a `week10_scrim.json` handoff without writing Week 10 scrim.
- **`week10_scrim.json`** — written by `/week10/scrim`, consuming Week 10 prep
  into a deterministic scrim protocol, readiness meters, lane-state pressure,
  scrim-effect deltas, and a `week10_match_plan.json` handoff without writing
  Week 10 match planning.
- **`week10_match_plan.json`** — written by `/week10/match`, consuming Week 10
...
- **`week11_match_plan.json`** — written by `/week11/match`, consuming Week 11
  ...
  `week11_match_result` boundary and `week11_match_result.json` handoff.
- **`week11_match_result.json`** — written by `/week11/match/result`, consuming
  Week 11 match plan into a deterministic result, scoreline, visible result
  effects, and causal chain while stopping before Week 11 post-match review.
