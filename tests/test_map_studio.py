...
from esports_sim.web.server import map_studio_create
...
def test_workbench_synthesis_and_roundtrip():
    # Synthesize ascent
    doc = map_workbench.synthesize_document("ascent")
    assert doc.id == "ascent"
    assert doc.display_name == "Ascent"
    assert len(doc.walkable_surfaces) > 0
    assert len(doc.semantic_zones) > 0

    # Compile synthesized document back to Map and MapGeometry
    map_obj, geo_obj = map_workbench.compile_document(doc)
    assert map_obj.id == "ascent"
    assert geo_obj.map_id == "ascent"
    assert len(map_obj.callouts) == len(doc.semantic_zones)
    assert len(geo_obj.regions) == len(doc.walkable_surfaces)
