...
def create_app(
    *,
    ...
    @app.route("/week11/scrim", methods=["GET", "POST"])
    def week11_scrim():
        decisions = require_decisions()
        if decisions is None:
            return redirect(url_for("practice"))
        result = run_slice(world, config, decisions, content_config=content_config)
        if result.week7_setup is None:
            flash("Week 11 scrim unlocks after Week 11 prep.")
            return redirect(url_for("practice"))
        ...
        missing = [
            name
            for name, path in (
                (WEEK11_PREP_FILENAME, week11_prep_path),
            )
            if not path.is_file()
        ]
        scrim_plan = None
        scrim_lock = None
        week11_scrim_written_path = ""
        if not missing:
            try:
                prep = week11_prep_from_json(week11_prep_path.read_text(encoding="utf-8"))
                scrim_plan = week11_scrim_plan(prep)
            except ValueError as exc:
                flash(str(exc))
            else:
                if request.method == "POST":
                    selected = (request.form.get("week11_scrim") or "").strip()
                    try:
                        scrim_lock = resolve_week11_scrim(scrim_plan, selected)
                    except ValueError:
                        flash("Choose a Week 11 scrim protocol.")
                    else:
                        week11_scrim_path.write_text(
                            render_week11_scrim_json(scrim_lock),
                            encoding="utf-8",
                            newline="\n",
                        )
                        week11_scrim_written_path = str(week11_scrim_path)
                elif week11_scrim_path.is_file():
                    try:
                        scrim_lock = week11_scrim_from_json(week11_scrim_path.read_text(encoding="utf-8"))
                    except ValueError as exc:
                        flash(str(exc))
                    else:
                        week11_scrim_written_path = str(week11_scrim_path)
    ...
    @app.route("/week11/match/result", methods=["GET", "POST"])
    def week11_match_result():
        decisions = require_decisions()
        if decisions is None:
            return redirect(url_for("practice"))
        result = run_slice(world, config, decisions, content_config=content_config)
        if result.week7_setup is None:
            flash("Week 11 result unlocks after Week 11 match planning.")
            return redirect(url_for("practice"))
        ...
        missing = [
            name
            for name, path in (
                (WEEK11_MATCH_PLAN_FILENAME, week11_match_plan_path),
            )
            if not path.is_file()
        ]
        plan = None
        match_result = None
        week11_match_result_written_path = ""
        if not missing:
            try:
                plan = week11_match_plan_from_json(week11_match_plan_path.read_text(encoding="utf-8"))
            except ValueError as exc:
                flash(str(exc))
            else:
                if request.method == "POST":
                    match_result = resolve_week11_match_result(plan)
                    week11_match_result_path.write_text(
                        render_week11_match_result_json(match_result),
                        encoding="utf-8",
                        newline="\n",
                    )
                    week11_match_result_written_path = str(week11_match_result_path)
                elif week11_match_result_path.is_file():
                    try:
