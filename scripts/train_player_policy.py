...
MotorRow = tuple[str, int, MotorDecisionTraceV1]
...
def _generate(
...
) -> tuple[list[ActionRow], list[CommunicationRow], list[MotorRow]]:
    gd = load_all()
    ...
    for map_id in map_ids:
        for seed in seeds:
            ...
            motors.extend((map_id, seed, trace) for trace in recorder.motor_traces)
    return actions, communications, motors
...
def _write_dataset(
    ...
    train_motors: list[MotorRow],
    validation_motors: list[MotorRow],
) -> None:
    ...
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        for split, kind, rows in (
            ...
            ("train", "motor", train_motors),
            ("validation", "motor", validation_motors),
        ):
...
def main() -> None:
    ...
    train_actions, train_comms, train_motors = _generate(map_ids, train_seeds)
    validation_actions, validation_comms, validation_motors = _generate(
        map_ids, validation_seeds
    )
    sampled_actions = _even_sample(train_actions, args.max_traces)
    sampled_motors = _even_sample(train_motors, args.max_traces)
    model = LearnedPlayerModel.train(
        [row[2] for row in sampled_actions],
        [row[2] for row in train_comms],
        [row[2] for row in sampled_motors],
    )
    ...
    motor_metrics = motor_imitation_metrics(
        model, [row[2] for row in validation_motors]
    )
    report = {
        "maps": map_ids,
